...
RUN python -m grpc_tools.protoc -I. --python_out=. --grpc_python_out=. vision_algorithms.proto
...
COPY --from=builder --chown=${USER}:${GROUP} ${WORKSPACE}/vision_algorithms.proto /
...
COPY --chown=${USER}:${GROUP} vision_algorithms_server.py ${WORKSPACE}/vision_algorithms_server.py
...
CMD ["python", "vision_algorithms_server.py"]